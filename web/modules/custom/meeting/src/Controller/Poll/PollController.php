...
class PollController extends ControllerBase
{
  ...
  public static function actionEdit($data)
  {
    ...
    $link_edit = Url::fromRoute('meeting.poll_get_by_id', ['id' => $data->id]);
    ...
    $link_url->setOptions([
      'attributes' => [
        ...
        'data-dialog-options' => Json::encode(['width' => '980', 'height'=>'auto', 'dialogClass'=>'custom-class-modal', 'show'=>'fadeIn', 'hide' =>'fadeOut'])
      ],
    ]);
    ...
    return  array(
      'data' => new FormattableMarkup(
        '<div class="action-dropdown">
           ...
           <button class="btn btn-sm btn--primary edit-poll-ajax"  data-url='.$link_edit->toString().' data-poll="@data_edit">@name_edit</button>
           ',
        [
  ...
  public function getById($id)
  {
    ...
    $query =  $query->select('poll', 'p')
      ->condition('id', $id)
      ->fields('p', ['id', 'poll_question'])
      ->execute()->fetchAssoc();
